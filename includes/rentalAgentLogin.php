...
if (isset($_POST['rentalAgentLogin'])) {
    ...
    if (!$hasErrors) {
        try {
            $select_statement_existing_customer = "
                ...
                WHERE ud.email = :email AND ud.role = 'rental-agent'
            ;";
            ...
            if ($user && password_verify($sanitized_password, $user['password'])) {
                ...
                <script>
                    Swal.fire({
                        ...
                        window.location.href = '../rentalAgentDashboard/rentalAgentDashboard.php';
                    });
                </script>";
                } else {
                    $rental_agent_login_message = "
            ...
            $rental_agent_login_message = "<p class='text-red-500 text-sm'>Database error: " . htmlspecialchars($e->getMessage()) . "</p>";
        }
    }
}
?>
...
<form method="POST" class="max-w-md w-full mx-auto bg-white/80 backdrop-blur-md p-8 rounded-xl shadow-lg border border-[#df927f]">
    <h2 class="text-2xl font-bold text-[#df927f] mb-6 text-center">Rental Agent Log-in</h2>
    ...
    <!-- Submit -->
    <button type="submit" name="rentalAgentLogin" class="w-full text-white bg-[#df927f] hover:bg-[#c97c65] focus:ring-4 focus:outline-none focus:ring-[#df927f]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center transition">
        Log-in
    </button>
...
<p>
    <?php echo $rental_agent_login_message; ?>
