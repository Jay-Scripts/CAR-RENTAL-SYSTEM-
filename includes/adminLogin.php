    ...
    if (!$hasErrors) {
        try {
            $select_statement_existing_customer = "
                ...
                WHERE ud.email = :email AND ud.role = 'admin'
            ;";
            ...
            if ($user && password_verify($sanitized_password, $user['password'])) {
                ...
                <script>
                    Swal.fire({
                        ...
                        window.location.href = '../adminDashboard/adminDashboard.php';
                    });
                </script>";
                } else {
                    $login_message = "
...
<form method="POST" class="max-w-md w-full mx-auto bg-white/80 backdrop-blur-md p-8 rounded-xl shadow-lg border border-[#df927f]">
    <h2 class="text-2xl font-bold text-[#df927f] mb-6 text-center">Admin Log-in</h2>
